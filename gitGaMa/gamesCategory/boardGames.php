        ...
        ?>
            <div class="honorableMentions">
                <div class="container">
                    ...
                    <h3><a href="/gitGaMa/gamesCategory/boardGames.php?genre=Strategy">STRATEGY</a>-<a href="/gitGaMa/gamesCategory/boardGames.php?genre=Strategy&tourney=yes"><i class="fas fa-trophy" style="color: orange"></i></a> 
                     <a href="/gitGaMa/gamesCategory/boardGames.php?genre=Puzzler">PUZZLER</a>-<a href="/gitGaMa/gamesCategory/boardGames.php?genre=Puzzler&tourney=yes"><i class="fas fa-trophy" style="color: orange"></i></a>
                     <a href="/gitGaMa/gamesCategory/boardGames.php?genre=Simulation">SIMULATION</a>-<a href="/gitGaMa/gamesCategory/boardGames.php?genre=Simulation&tourney=yes"><i class="fas fa-trophy" style="color: orange"></i></a></h3>
                    <ul>
                    <?php
                    //partea de filtru
                    if (isset($_GET['genre'])) {
                        ...
                        {
                            if($hasTourney==1)
                                $sqlTopGamesDesc= "SELECT * FROM games WHERE typegame='Board' and genre ='$genre' and tourney is true ORDER BY likes DESC";
                            else
                                $sqlTopGamesDesc = "SELECT * FROM games WHERE typegame='Board' and genre ='$genre' ORDER BY likes DESC";
                        }
                        //Daca e user obisnuit i le afiseaza doar pe cele la care se incadreaza cu varsta
                        else 
                        {
                            if($hasTourney==1)
                                $sqlTopGamesDesc = "SELECT * FROM games WHERE minimumAge<=$row[0] AND maximumAge>=$row[0] AND typegame='Board' and genre ='$genre' and tourney is true ORDER BY likes DESC";
                            else
                                $sqlTopGamesDesc = "SELECT * FROM games WHERE minimumAge<=$row[0] AND maximumAge>=$row[0] AND typegame='Board' and genre ='$genre' ORDER BY likes DESC";
                        }
                        ...
                        if ($name == 'admin')
                            $sqlTopGamesDesc = "SELECT * FROM games WHERE typegame='Board' ORDER BY likes DESC";
                        //Daca e user obisnuit i le afiseaza doar pe cele la care se incadreaza cu varsta
                        else $sqlTopGamesDesc = "SELECT * FROM games WHERE minimumAge<=$row[0] AND maximumAge>=$row[0] AND typegame='Board' ORDER BY likes DESC";
                        $result2 = mysqli_query($conn, $sqlTopGamesDesc);
